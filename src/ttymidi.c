...
void parse_midi_command(snd_seq_t* seq, int port_out_id, char *buf, int buflen)
{
	/*
	   MIDI COMMANDS
	   -------------------------------------------------------------------
...
{
	char buf[BUF_SIZE], msg[MAX_MSG_SIZE];
	int i, buflen = 0, msglen, bytesleft;
	...
	while (run) 
	{
		...
		while (i < bytesleft) {
			...
			if (buf[i] >> 7 != 0) {
				...
				if(buf[i] == 0xF7 && buf[0] == 0xF0)	//if end of SysEx message has been reached
				{
					break;
				}
				buf[0] = buf[i];
				if(buf[0] != 0xF0)	//if not SysEx
					bytesleft = 3;
				i = 1;
			} else {
				if(buf[0] == 0xF0)	//if SysEx
				{
					i++;
				}
				else
		...
		/* parse MIDI message */
		else parse_midi_command(seq, port_out_id, buf, i+1);
	}
}
